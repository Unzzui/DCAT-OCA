...
export default function LecturasPage() {
  ...
  const fetchPeriodos = useCallback(async () => {
    try {
      const response = await api.get<{ meses: number[]; anios: number[]; ultimo_mes: number | null; ultimo_anio: number | null }>('/api/v1/lecturas/periodos')
  ...
  useEffect(() => {
    const loadAll = async () => {
      setLoading(true)
      const [, , , periodosRes] = await Promise.all([fetchSectores(), fetchHallazgos(), fetchInspectors(), fetchPeriodos()])
      ...
      const [statsRes, dataRes] = await Promise.all([
        api.get<Stats>(`/api/v1/lecturas/stats${statsParams.toString() ? '?' + statsParams.toString() : ''}`),
        api.get<PaginatedResponse>(`/api/v1/lecturas?${dataParams.toString()}`),
      ])
      setStats(statsRes)
      setData(dataRes)
