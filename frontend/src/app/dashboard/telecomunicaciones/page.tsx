...
export default function TelecomunicacionesPage() {
  ...
  const fetchPeriodos = useCallback(async () => {
    try {
      const response = await api.get<{ meses: number[]; anios: number[]; ultimo_mes: number | null; ultimo_anio: number | null }>('/api/v1/teleco/periodos')
  ...
  useEffect(() => {
    const loadAll = async () => {
      setLoading(true)
      const [, , , periodosRes] = await Promise.all([fetchEmpresas(), fetchComunas(), fetchInspectors(), fetchPeriodos()])
