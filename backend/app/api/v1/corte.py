...
async def get_corte(
    ...
    return await corte_service.get_corte_filtered_data(
        search=search, zona=zona, centro_operativo=centro_operativo,
        comuna=comuna, inspector=inspector, situacion_encontrada=situacion_encontrada,
        motivo_multa=motivo_multa, mes=mes, anio=anio,
...
@router.get("/stats")
async def get_stats(
    zona: Optional[str] = Query(None), centro_operativo: Optional[str] = Query(None),
    comuna: Optional[str] = Query(None), inspector: Optional[str] = Query(None),
    mes: Optional[int] = Query(None), anio: Optional[int] = Query(None),
    current_user: User = Depends(get_current_user),
) -> Dict[str, Any]:
    return await corte_service.get_corte_stats(
...
async def get_zonas(current_user: User = Depends(get_current_user)):
    return await corte_service.get_corte_zonas()


@router.get("/centros-operativos", response_model=List[str])
async def get_centros_operativos(current_user: User = Depends(get_current_user)):
    return await corte_service.get_corte_centros_operativos()


@router.get("/comunas", response_model=List[str])
async def get_comunas(current_user: User = Depends(get_current_user)):
    return await corte_service.get_corte_comunas()


@router.get("/inspectores", response_model=List[Dict[str, Any]])
async def get_inspectores(current_user: User = Depends(get_current_user)):
    return await corte_service.get_corte_inspectores()


@router.get("/situaciones", response_model=List[str])
async def get_situaciones(current_user: User = Depends(get_current_user)):
    return await corte_service.get_corte_situaciones()


@router.get("/periodos")
async def get_periodos(current_user: User = Depends(get_current_user)) -> Dict[str, Any]:
    return await corte_service.get_corte_periodos()
...
async def export_data(
    ...
    zona: Optional[str] = None, centro_operativo: Optional[str] = None,
    comuna: Optional[str] = None, inspector: Optional[str] = None,
    situacion_encontrada: Optional[str] = None, motivo_multa: Optional[str] = None,
    mes: Optional[int] = None, anio: Optional[int] = None,
    current_user: User = Depends(get_current_user),
):
    result = await corte_service.get_corte_filtered_data(
        search=search, zona=zona, centro_operativo=centro_operativo,
        comuna=comuna, inspector=inspector, situacion_encontrada=situacion_encontrada,
        motivo_multa=motivo_multa, mes=mes, anio=anio, page=1, limit=100000,
    )
    df = pd.DataFrame(result["items"])
    if df.empty:
        raise HTTPException(status_code=404, detail="No hay datos para exportar")
    if format == "excel":
        output = create_formatted_excel(df=df, sheet_name="Corte y Reposicion",
                                        title="Informe de Corte y Reposición",
                                        column_config=get_column_config_corte())
        return StreamingResponse(output,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": "attachment; filename=corte_reposicion.xlsx"})
    else:
        output = io.StringIO()
        df.to_csv(output, index=False)
        output.seek(0)
        return StreamingResponse(iter([output.getvalue()]), media_type="text/csv",
            headers={"Content-Disposition": "attachment; filename=corte_reposicion.csv"})
